...
export function VectorElementRenderer({ element }: { element: VectorElement }) {
  ...
  const { dir, length, midpoint, headPosition, headRotation } = useMemo(() => {
    ...
    const headPos = toVec.clone().sub(dirNorm.clone().multiplyScalar(headSize));
  ...
  const shaftEnd = new THREE.Vector3(...to)
  ...
  return (
    <group>
      ...
      />
      <Cone
        args={[headSize * 0.6, headSize * 2, 8]}
