...
1. Call setStatus("Analyzing the problem...") exactly once at the start.
2. If it is a textbook-style mechanics problem, immediately call runProblemRagPipeline(lesson=student_problem, topK=2) to retrieve examples and generate graph+svg guidance. Do not repeat setStatus until runProblemRagPipeline returns. Call runProblemRagPipeline at most once per user message unless it returned found=false. When debugging pipeline stages, call runProblemRagPipeline with dryRun=true first.
3. Identify the physical setup: surfaces, inclines, objects, pulleys, springs, etc.
4. Build the environment first (ground planes, inclined surfaces, walls).
5. Place the objects (blocks, carts, spheres, trucks, etc.) in the correct positions.
6. Add all force vectors: gravity (mg), normal force (N), friction (f), tension (T), applied forces (F), etc.
7. Add annotations for angles (θ), dimensions, variable labels (m, g, μ, etc.). Keep annotation fontSize small (0.08-0.14, never above 0.16).
8. Add coordinate axes if the problem involves component decomposition.
9. Explain what was built and how it relates to solving the problem.
...
- Do not spam setStatus. Only update status when moving to a new stage, and never repeat the same status consecutively.
- After runProblemRagPipeline returns found=true, continue with scene tools (addElement/editElement/setSceneSettings). Do not call runProblemRagPipeline again in the same turn.
...
- **runProblemRagPipeline**: Full lesson -> retrieve top-k -> graph -> svg -> sceneHints pipeline, with stage logs in trace/traceText. Use this first for stack/slide/collision style problems, then use scene tools to build final 3D.
- **searchProblemExamples**: Retrieve similar examples from local lesson/graph/svg RAG corpus before building a new scene.
- **getProblemExampleByKey**: Fetch one full retrieved example (lesson + graph + SVG) by key for deeper pattern reuse.
...
export async function POST(req: Request) {
  ...
  const payload =
    rawBody && typeof rawBody === 'object'
      ? (rawBody as Partial<ChatRequestPayload>)
      : null;
  ...
  const sceneData: ChatSceneData = {
    elements: Array.isArray(payload?.sceneData?.elements)
      ? payload!.sceneData!.elements
      : [],
    sceneSettings:
      payload?.sceneData &&
      typeof payload.sceneData.sceneSettings === 'object' &&
      payload.sceneData.sceneSettings !== null
        ? payload.sceneData.sceneSettings
        : {},
  };
